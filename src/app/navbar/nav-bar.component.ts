...
export class NavBarComponent implements OnInit, DoCheck {
  ...
  buildMsgLine(msg): any {
    ...
    const otherUserId = userArray[0];

    // let's build a thumbnail based on this user's id:

    const otherUser = this.userService.getUserFromMemoryById(otherUserId);
    ...
    const thumbnailObj = {
      user: otherUser, userId: otherUserId, online: false,
  ...
  removeItemFromMainList(msgItem): void {
