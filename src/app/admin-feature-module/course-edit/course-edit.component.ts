...
export class CourseEditComponent implements OnInit {
    ...
    buildMaterialsSubSection(value): FormGroup {
        return this.fb.group({
            material: value
        });
    }
    ...
    lintMe(combinedCourseObject): void {
        let lintedModel = combinedCourseObject;
        //   console.log('LINTING: ');

        combinedCourseObject.sections.forEach((combCO) => {
            const sectionContent = combCO.content;

            let LintedSectionContent = sectionContent;
            if (sectionContent) {
                LintedSectionContent = sectionContent.replace(/\n/g, '<br>');
            }
            combCO.content = LintedSectionContent;
        });
