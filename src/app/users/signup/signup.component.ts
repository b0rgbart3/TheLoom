...
export class SignupComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.user) {
      ...
      this.user = new User('', '', '', '', '', '', '', '', '', '', '', false, false, false, false, '', '', '', '', '', [], []);
  ...
  //     .then(response => {
  //       this.fbStatusResponseObject = response;
      ...
      if (this.user.id === '0' || this.user.id === undefined) {
        ...
        this.userService.createUser(combinedObject).subscribe(
          (val) => {
            ...
            // }
            // this.notes.add(new LoomNotification('success', ['Welcome to the ReclaimingLoom, ' +
